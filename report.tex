...
\author{%
  ...
  \alignauthor{Andy Lum\\
    \affaddr{Lakeland, TN}\\
    \email{andylum77@yahoo.com}}\\
}

\maketitle

\begin{abstract}
  UPDATED---\today. In our senior design project, we aim to develop an interactive dashboard for visualizing and accessing solar irradiance data from the West Tennessee Solar Farm in Stanton, Tennessee. This project aligns with the University of Tennessee Research Foundation's interest in rejuvenating the site's potential, given its proximity to the new Ford plant. Presently, the existing interactive dashboard on the farm's website is nonfunctional.

To address this, we will utilize a combination of R, Shiny, Python, and SQL technologies. By utilizing simulated solar irradiance data and a hypothetical sensor layout, we intend to create a comprehensive representation of the solar farm's data collection system. Collaborating with Dr. Sims, we are actively procuring actual irradiance measurements gathered at the site.

Our approach involves Python for the simulation of real-time data collection, while R and Shiny will be utilized to craft an interactive and visually appealing dashboard. Our ultimate goal is to seamlessly integrate this dashboard into a dedicated website, ensuring that the valuable insights it offers become widely accessible to the general public.
\end{abstract}
